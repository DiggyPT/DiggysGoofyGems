...
public class BlockTurboCounter extends Block implements ITileEntityProvider, IHasModel {

    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);

    public BlockTurboCounter(String name, Material material, float hardness) {
    	super(material);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
        ...
        if (!world.isRemote) {
        	...
        	//basically checks where the block is facing and does the function accordingly
                ...
                if (state.getValue(FACING) == EnumFacing.NORTH) {
                	if (hitY < .5f) {
                		if (hitX < .5f) {
                			...
                			counter = getTE(world, pos).divide();
                		}
                	} else {
                		if (hitX < .5f) {
                			counter = getTE(world, pos).increment();
                		} else {
                			counter = getTE(world, pos).duplicate();
                		}
                	}
